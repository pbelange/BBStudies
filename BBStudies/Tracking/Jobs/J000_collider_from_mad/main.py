...
def build_collider_from_mad(config_mad, sanity_checks=True):
    ...
    # Build sequences
    ost.build_sequence(mad_b1b2 , mylhcbeam=1   ,slice_factor = config_mad['slice_factor'])
    ost.build_sequence(mad_b4   , mylhcbeam=4   ,slice_factor = config_mad['slice_factor'])
...
def build_collider(config_file="config.yaml"):
    ...
    # Save to file
    xutils.mkdir(config_mad['save_collider'])
    
    # Saving config in metadata
    collider.metadata['config_J000'] = config_mad
    collider.to_json(config_mad['save_collider'])
...
if __name__ == '__main__':
    ...
    build_collider(config_file=args.config)
    #===========================
